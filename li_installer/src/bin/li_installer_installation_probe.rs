...
fn dependency_version(name: &str, path: &str, dependencies: &BTreeMap<String, String>) -> String {
    match name {
        "apt_get" | "brew" | "cc" | "cmake" | "ctest" | "curl" | "dnf" | "docker" | "loginctl"
        | "openssl" | "pacman" | "python" | "sg" | "stat" | "sudo" | "systemctl"
        | "systemd_run" | "tar" | "zypper" => {
...
fn run() -> Result<(), String> {
    let arguments = ProbeArguments::parse(&env::args().skip(1).collect::<Vec<_>>())?;
    println!("{}", installation_probe(&arguments)?.render(0));
    eprintln!("{}", PLATFORM_PROBE_COMPLETE_EVENT);
    Ok(())
}

// Converts the native provider result into one stable process exit status.
fn main() {
    if let Err(error) = run() {
        eprintln!("installation probe: {}", error);
        std::process::exit(1);
    }
}
