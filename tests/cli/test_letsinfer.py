...
class RuntimeCandidateCliTests(unittest.TestCase):
    ...
    def test_node_agent_memory_envelope_supports_runtime_staging(self) -> None:
        ...
        self.assertIn("MemoryMin=134217728\n", unit)
        self.assertIn("MemoryLow=134217728\n", unit)
        self.assertIn("MemoryHigh=134217728\n", unit)
        self.assertIn("MemoryMax=201326592\n", unit)
        self.assertIn("OOMPolicy=continue\n", unit)

    def test_runtime_floor_cannot_weaken_core_control_plane_reserve(self) -> None:
        manifest = cli.runtime_execution_manifest(
            runtime_candidate(), qualified=True
        )
        reserve = cli.effective_memory_reserve(manifest, phase="runtime")
        self.assertEqual(reserve["runtime_minimum_bytes"], 1 * 1024**3)
        self.assertEqual(reserve["core_minimum_bytes"], 16 * 1024**3)
        self.assertEqual(reserve["effective_bytes"], 16 * 1024**3)

        safe = {
            "memory_model": "unified",
            "host_available_bytes": 17 * 1024**3,
            "host_available_gib": 17,
            "host_total_bytes": 24 * 1024**3,
            "host_total_gib": 24,
        }
        with mock.patch.object(cli, "memory_snapshot", return_value=safe):
            admitted = cli.require_memory_reserve(manifest, phase="runtime")
        self.assertEqual(admitted["maximum_engine_bytes"], 8 * 1024**3)

        unsafe = {**safe, "host_total_bytes": 23 * 1024**3, "host_total_gib": 23}
        with (
            mock.patch.object(cli, "memory_snapshot", return_value=unsafe),
            self.assertRaisesRegex(cli.LetsInferError, "exceeds the safe host budget"),
        ):
            cli.require_memory_reserve(manifest, phase="runtime")

    def test_engine_container_contract_bounds_helpers_and_prefers_engine_oom(self) -> None:
        memory_max = 8 * 1024**3
        memory_soft = cli.engine_memory_soft_bytes(memory_max)
        manifest = {"container": {"memory_bytes": memory_max}}
        inspection = {
            "HostConfig": {
                "Memory": memory_max,
                "MemoryReservation": memory_soft,
                "MemorySwap": memory_max,
                "OomScoreAdj": cli.ENGINE_OOM_SCORE_ADJUST,
                "OomKillDisable": False,
            }
        }
        contract = cli.require_container_memory_contract(inspection, manifest)
        self.assertEqual(contract["memory_soft_bytes"], 7 * 1024**3)
        inspection["HostConfig"]["MemorySwap"] = 2 * memory_max
        with self.assertRaisesRegex(cli.LetsInferError, "containment contract"):
            cli.require_container_memory_contract(inspection, manifest)

    def test_every_engine_launch_applies_the_process_tree_memory_boundary(self) -> None:
        manifest = cli.runtime_execution_manifest(
            runtime_candidate(), qualified=True
        )
        with tempfile.TemporaryDirectory() as directory:
            root = pathlib.Path(directory)
            artifact = root / "objects" / ("1" * 64)
            artifact.mkdir(parents=True)
            with mock.patch.object(cli, "default_runtime_home", return_value=root):
                command = cli.docker_command(
                    manifest,
                    name="letsinfer-test",
                    ...
                    runtime_cache_root=root / "runtime",
                    api_key_file=root / "engine.key",
                    tls_cert_file=root / "server.crt",
                    tls_key_file=root / "server.key",
                    runtime_artifact_root=artifact,
                )

        def option(name: str) -> str:
            return command[command.index(name) + 1]

        memory_max = manifest["container"]["memory_bytes"]
        self.assertEqual(option("--memory"), str(memory_max))
        self.assertEqual(
            option("--memory-reservation"),
            str(cli.engine_memory_soft_bytes(memory_max)),
        )
    ...
    def test_last_watchdog_incident_survives_trip_acknowledgement(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            for name in (cli.PROTECTION_TRIP_NAME, cli.PROTECTION_INCIDENT_NAME):
                path = root / name
                path.write_text(json.dumps(incident), encoding="utf-8")
                path.chmod(0o600)
            config = {"protection_root": str(root)}
            self.assertTrue(cli.clear_protection_trip(config))
            self.assertFalse((root / cli.PROTECTION_TRIP_NAME).exists())
            self.assertEqual(
                cli.read_protection_incident(config)["reason"],
